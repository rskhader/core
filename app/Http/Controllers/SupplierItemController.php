...
class SupplierItemController extends Controller
{
    ...
    {
//        'id', 'supplier_item_num', 'phone', 'address', 'supplier_item_name'
        ...
        $supplier_item->supplier_item_num = $request->input('supplier_item_num');
        $supplier_item->supplier_item_name = $request->input('supplier_item_name');
        $supplier_item->address = $request->input('address');
        $supplier_item->phone = $request->input('phone');
        $request = $supplier_item->save();

        if ($request === TRUE) {
            return redirect('inventory/supplier_item')->with('success', 'تم حفظ المورد بنجاح *');
        } else {
